...
# Model
model = Sequential()
model.add(Dense(units=100, input_shape=(9,), activation=activacion, kernel_regularizer=l2(landa), kernel_initializer=inicializacion))
...
model.add(Dense(units=4, activation='softmax', kernel_regularizer=l2(landa), kernel_initializer=inicializacion))
...
print('Precision: %.2f, %.2f, %.2f, %.2f' % (prec_val[0], prec_val[1], prec_val[2], prec_val[3]))
print('Recall:    %.2f, %.2f, %.2f, %.2f' % (rec_val[0], rec_val[1], rec_val[2], rec_val[3]))
...
if ((prec_test[0]>u) and (prec_test[1]>u) and (prec_test[2]>u) and (prec_test[3]>u) and (rec_test[0]>u) and (rec_test[1]>u) and (rec_test[2]>u) and (rec_test[3]>u)):
    if ((prec_val[0]>u) and (prec_val[1]>u) and (prec_val[2]>u) and (prec_val[3]>u) and (rec_val[0]>u) and (rec_val[1]>u) and (rec_val[2]>u) and (rec_val[3]>u)):
