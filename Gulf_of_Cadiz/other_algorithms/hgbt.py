...
# Apply histogram-based gradient boosted trees
print('\nApplying HGBT')
tini = time.time()
gbdt = HistGradientBoostingClassifier(max_iter=500)
gbdt.fit(X_train, ok_train)
tfin = time.time()
print('Runtime: %.1f' %(tfin-tini))

pred_train = gbdt.predict(X_train)
...
if ((prec_test[0]>u) and (prec_test[1]>u) and (prec_test[2]>u) and (prec_test[3]>u) and (rec_test[0]>u) and (rec_test[1]>u) and (rec_test[2]>u) and (rec_test[3]>u)):
    ...
    os.mkdir(nombre_dir)
    joblib.dump(gbdt, nombre_dir+"/hgbt.gz")
