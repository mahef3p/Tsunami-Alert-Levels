...
train_clase = np.zeros(4)
val_clase = np.zeros(4)
test_clase = np.zeros(4)
for i in range(4):
...
print('Precision: %.2f, %.2f, %.2f, %.2f' % (prec_test[0], prec_test[1], prec_test[2], prec_test[3]))
print('Recall:    %.2f, %.2f, %.2f, %.2f' % (rec_test[0], rec_test[1], rec_test[2], rec_test[3]))

# Show accuracy per class
labels = ['Green', 'Yellow', 'Orange', 'Red']
ancho_barra = 2.0
ancho_linea = 0.5
x = np.arange(0, 23, 7.5)
...
autolabel(rect2)
autolabel(rect3)
fig.set_size_inches(21.0/2.54, 12.2/2.54)
fig.tight_layout()
plt.show()
